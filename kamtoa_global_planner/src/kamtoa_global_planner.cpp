...
#include "kamtoa_global_planner.h"

#include <pluginlib/class_list_macros.h>
//register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(kamtoa_planner::KamtoaGlobalPlanner, nav_core::BaseGlobalPlanner)
...
namespace kamtoa_planner
{

//Default Constructor
KamtoaGlobalPlanner::KamtoaGlobalPlanner()
{

}
KamtoaGlobalPlanner::KamtoaGlobalPlanner(ros::NodeHandle &nh)
{
  ROSNodeHandle = nh;

}

KamtoaGlobalPlanner::KamtoaGlobalPlanner(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  initialize(name, costmap_ros);
}

void KamtoaGlobalPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
  if (!initialized_)
  {
    ...
    private_nh.param<std::string>("wording", s, "noooo!");
    std::cout << "Hi, Kamtoa Kamtoa!! : " << s << std::endl;
...
bool KamtoaGlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal,
                             std::vector<geometry_msgs::PoseStamped>& plan)
{
  std::cout << "Kamtoa makePlan" << std::endl;
  if (!initialized_)
  {
...
void KamtoaGlobalPlanner::getCorrdinate(float& x, float& y)
{
...
int KamtoaGlobalPlanner::convertToCellIndex(float x, float y)
{
...
void KamtoaGlobalPlanner::convertToCoordinate(int index, float& x, float& y)
{
...
bool KamtoaGlobalPlanner::isCellInsideMap(float x, float y)
{
  bool valid = true;
...
void KamtoaGlobalPlanner::mapToWorld(double mx, double my, double& wx, double& wy){
   costmap_2d::Costmap2D* costmap = costmap_ros_->getCostmap();
    wx = costmap->getOriginX() + mx * resolution;
    wy = costmap->getOriginY() + my * resolution;
}

vector<int> KamtoaGlobalPlanner::RAstarPlanner(int startCell, int goalCell){
...
/*********************************************************************************/
vector<int> KamtoaGlobalPlanner::findPath(int startCell, int goalCell, float g_score[])
{
  value++;
  vector<int> bestPath;
...
/*********************************************************************************/
vector<int> KamtoaGlobalPlanner::constructPath(int startCell, int goalCell,float g_score[])
{
  vector<int> bestPath;
  vector<int> path;
...
/*********************************************************************************/
/*
float KamtoaGlobalPlanner::calculateHCost(int cellID, int goalCell)
{    
...
/*********************************************************************************/
void KamtoaGlobalPlanner::addNeighborCellToOpenList(multiset<cells> & OPL, int neighborCell, int goalCell, float g_score[])
{
  cells CP;
...
vector <int> KamtoaGlobalPlanner::findFreeNeighborCell (int CellID){
...
/*********************************************************************************/
bool KamtoaGlobalPlanner::isStartAndGoalCellsValid(int startCell,int goalCell)
{ 
 bool isvalid=true;
 ...
 float  KamtoaGlobalPlanner::getMoveCost(int i1, int j1, int i2, int j2){
  ...
  float  KamtoaGlobalPlanner::getMoveCost(int CellID1, int CellID2){
 ...
 bool  KamtoaGlobalPlanner::isFree(int i, int j){
 ...
 return OGM[CellID];
  ...
  //verify if the cell(i,j) is free
 bool  KamtoaGlobalPlanner::isFree(int CellID){
 return OGM[CellID];
 } 
}
;
